...
Node* Node::eval() {
    ...
    //First, recursively evaluate the macrocell node's four children nodes (if this node is not a leaf, in which case default to the manual way)
    if(this->depth > 2) {
    ...
    //Search was not successful
        if (depth > 2) {
            ...
            //Insert this node into the hashtable for reuse
